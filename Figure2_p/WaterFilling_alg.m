function [ pS,pR,pO,pQ ] = WaterFilling_alg( H11,H22,H33,H44,H12,H13,H14,H21,H23,H24,H31,H32,H34,H41,H42,H43,beta1,beta2,beta3,beta4,sigma_n,iter_num,P1_max,P2_max,P3_max,P4_max,e1,e2,e3,e4 )
...
%% Update p1£¬p2 with lamda1,lamda2 fixed
  [ mu1,mu2,mu3,mu4 ] = get_mu( p1,p2,p3,p4,sigma_n,beta1,beta2,beta3,beta4,H11,H22,H33,H44,H12,H13,H14,H21,H23,H24,H31,H32,H34,H41,H42,H43,P1_max,P2_max,P3_max,P4_max );
  ...
  A1 = ( muS( :,iter ) + gammaS( :,iter ) )^(-1) - ( beta1 * trace( H12 * H12') * pR( :,iter ) + beta1 * trace( H13 * H13') * pO( :,iter ) + beta1 * trace( H14 * H14') * pQ( :,iter ) + sigma_n ) / ( beta1 * trace( H11 * H11') );
    if A1 <= 0
       pS( :,iter + 1 ) = 0;
    else 
       if A1 >= P1_max
 ...
 A2 = ( muR( :,iter ) + gammaR( :,iter ) )^(-1) - ( beta2 * trace( H21 * H21') * pS( :,iter + 1 ) + beta2 * trace( H23 * H23') * pO( :,iter ) + beta2 * trace( H24 * H24') * pQ( :,iter ) + sigma_n ) / ( beta2 * trace( H22 * H22') );
    if A2 <= 0
       pR( :,iter + 1 ) = 0;
    else
       if A2 >= P2_max
  ...
  A3 = ( muO( :,iter ) + gammaO( :,iter ) )^(-1) - ( beta3 * trace( H31 * H31') * pS( :,iter + 1 ) + beta3 * trace( H32 * H32') * pR( :,iter + 1 ) + beta3 * trace( H34 * H34') * pQ( :,iter ) + sigma_n ) / ( beta3 * trace( H33 * H33') );
    if A3 <= 0
       pO( :,iter + 1 ) = 0;
    else 
       if A3 >= P3_max
  ...
  A4 = ( muQ( :,iter ) + gammaQ( :,iter ) )^(-1) - ( beta4 * trace( H41 * H41') * pS( :,iter + 1 ) + beta4 * trace( H42 * H42') * pR( :,iter + 1 ) + beta4 * trace( H43 * H43') * pO( :,iter + 1 ) + sigma_n ) / ( beta4 * trace( H44 * H44') );
    if A4 <= 0
       pQ( :,iter + 1 ) = 0;
    else
       if A4 >= P4_max
...
%% Update lamda1,lamda2 with p1£¬p2 fixed
   ...
   g1( :,iter ) = ( 1 - beta1 ) * trace( H11 * H11') * pS( :,iter ) - e1;
   g2( :,iter ) = ( 1 - beta2 ) * trace( H22 * H22') * pR( :,iter ) - e2;
   g3( :,iter ) = ( 1 - beta3 ) * trace( H33 * H33') * pO( :,iter ) - e3;
   g4( :,iter ) = ( 1 - beta4 ) * trace( H44 * H44') * pQ( :,iter ) - e4;
