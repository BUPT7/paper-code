function [ pS,pR] = WaterFilling_alg( H11,H22,H12,H21,beta1,beta2,sigma_n,iter_num,P1_max,P2_max,e,aL)
...
%% Update p1£¨p2 with lamda1,lamda2 fixed
  ...
  muS( :,iter ) = mu1;
  muR( :,iter ) = mu2;
  gammaS( :,iter ) = lamdaS( :,iter ) * ( 1 - beta1 ) * c;
  gammaR( :,iter ) = lamdaS( :,iter ) * ( 1 - beta2 ) * d;
...
% Update lamda1,lamda2 with p1£¨p2 fixed
   ...
   g1( :,iter ) = ( 1 - beta1 ) * c * pS( :,iter ) - e;
   g2( :,iter ) = ( 1 - beta2 ) * d * pR( :,iter ) - e;
