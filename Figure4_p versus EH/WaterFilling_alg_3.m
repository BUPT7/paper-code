function [ pS,pR,pO ] = WaterFilling_alg_3( H11,H22,H33,H12,H13,H21,H23,H31,H32,beta1,beta2,beta3,sigma_n,iter_num,P1_max,P2_max,P3_max,eL )
...
%% Update p1£¬p2 with lamda1,lamda2 fixed
  ...
  [ mu1,mu2,mu3] = get_mu3( p1,p2,p3,sigma_n,beta1,beta2,beta3,H11,H22,H33,H12,H13,H21,H23,H31,H32,P1_max,P2_max,P3_max,gammaS( :,iter ),gammaR( :,iter ),gammaO( :,iter ) );
...
%   gammaO( :,iter ) = lamdaO( :,iter ) * ( 1 - beta3 ) * trace( H33 * H33');
 ...
 A2 = ( muR( :,iter ) + gammaR( :,iter ) )^(-1) - ( beta2 * trace( H12 * H12') * pS( :,iter + 1 ) + beta2 * trace( H32 * H32') * pO( :,iter )  + sigma_n ) / ( beta2 * trace( H22 * H22') );
  ...
  A3 = ( muO( :,iter ) + gammaO( :,iter ) )^(-1) - ( beta3 * trace( H13 * H13') * pS( :,iter + 1 ) + beta3 * trace( H23 * H23') * pR( :,iter + 1 ) + sigma_n ) / ( beta3 * trace( H33 * H33') );
   ...
   g1( :,iter ) = ( 1 - beta1 ) * (trace( H11 * H11') * pS( :,iter ) + trace( H21 * H21') * pR( :,iter ) + trace( H31 * H31') * pO( :,iter )) - eL;
   g2( :,iter ) = ( 1 - beta2 ) * (trace( H22 * H22') * pR( :,iter)  + trace( H12 * H12') * pS( :,iter ) + trace( H32 * H32') * pO( :,iter )) - eL;
   g3( :,iter ) = ( 1 - beta3 ) * (trace( H33 * H33') * pO( :,iter ) + trace( H13 * H13') * pS( :,iter ) + trace( H23 * H23') * pR( :,iter )) - eL;
