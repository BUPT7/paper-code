...
sigma_1 = 1/SNR;
sigma_2 = 1/SNR;
sigma_n=(sqrt(sigma_1)+sigma_2)^2;
% sigma_n = 1/SNR;
d = 1;
...
if schemes(1,1)
 ...
 if schemes(1,2)
     ...
     for iter = 1 : iter_num + 1
      ...
      [ rate3_temp ] = get_rate2( p1( 1,2,iter ),p2( 1,2,iter ),p3( 1,2,iter ),H11,H22,H33,H12,H13,H21,H23,H31,H32,sigma_n,beta1,beta2,beta3 );
    ...
    if isMonte
       ...
       %% noise
             n_1 =(randn(N,1)+1i*randn(N,1)) /sqrt(2);
             n_1=(1/sqrt(SNR))*n_1;
            n_2 =(randn(N,1)+1i*randn(N,1)) /sqrt(2);
             n_2=(1/sqrt(SNR))*n_2;
%              n_r = (randn(N,1)+1i*randn(N,1)) /sqrt(2);
%              n_r = (1/sqrt(SNR))*n_r;
           n_r=sqrt(beta1)*n_1 + n_2;
       %% Symbols, QPSK
            c1 = zeros(M,2);
            c2 = zeros(M,2);
            c3 = zeros(M,2);
...
%             c3(1,1) = randi(2,[1 1])-1;%create 0 1;
           ...
           if schemes(1,1)
                [err1_temp ] =  get_err1(p1( 1,1,iter ),p2( 1,1,iter ),p3( 1,1,iter ),H11,H22,H33,H12,H13,H21,H23,H31,H32,beta1,beta2,beta3,c1,c2,c3,n_r);
           ...
           if schemes(1,2)
               for iter = 1 : iter_num + 1
                    [ err2_temp ] =  get_err1(p1_wf1( :,iter ),p2_wf1( :,iter ),p3_wf1( :,iter ),H11,H22,H33,H12,H13,H21,H23,H31,H32,beta1,beta2,beta3,c1,c2,c3,n_r );
