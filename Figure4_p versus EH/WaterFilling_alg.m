function [ pS,pR,pO,pQ] = WaterFilling_alg( H11,H22,H33,H44,H12,H13,H14,H21,H23,H24,H31,H32,H34,H41,H42,H43,beta1,beta2,beta3,beta4,sigma_n,iter_num,P1_max,P2_max,P3_max,P4_max,eL)
...
for iter = 1 : iter_num
    ...
    fprintf( 'p1 = %3.2f; p2 = %3.2f;p3 = %3.2f; p4 = %3.2f;...',p1,p2)
%% Update p1£¬p2 with lamda1,lamda2 fixed
  ...
  fprintf( 'mu1 = %5.4f; mu2 = %5.4f;mu3 = %5.4f; mu4 = %5.4f;...',mu1,mu2 );
  muS( :,iter ) = mu1;
  muR( :,iter ) = mu2;
muO( :,iter ) = mu3;
muQ( :,iter ) = mu4;
...
gammaO( :,iter ) = lamdaO( :,iter ) * ( 1 - beta2 ) * trace( H33 * H33');
gammaQ( :,iter ) = lamdaQ( :,iter ) * ( 1 - beta2 ) * trace( H44 * H44');
 ...
 A2 = ( muR( :,iter ) + gammaR( :,iter ) )^(-1) - ( beta2 * trace( H12 * H12') * pS( :,iter + 1 )+ beta1 * trace( H32 * H32') * pO( :,iter )+ beta1 * trace( H42 * H42') * pQ( :,iter ) + sigma_n ) / ( beta2 * trace( H22 * H22') );
    if A2 <= 0
       pR( :,iter + 1 ) = 0;
    else
       if A2 >= P2_max
 ...
 A3 = ( muO( :,iter ) + gammaO( :,iter ) )^(-1) - ( beta2 * trace( H13 * H13') * pS( :,iter + 1 )+ beta1 * trace( H23 * H23') * pR( :,iter+1 )+ beta1 * trace( H43 * H43') * pQ( :,iter ) + sigma_n ) / ( beta2 * trace( H33 * H33') );
    if A3 <= 0
       pO( :,iter + 1 ) = 0;
    else
       if A3 >= P3_max
 ...
 A4 = ( muQ( :,iter ) + gammaQ( :,iter ) )^(-1) - ( beta2 * trace( H14 * H14') * pS( :,iter + 1 )+ beta1 * trace( H24 * H24') * pR( :,iter+1 )+ beta1 * trace( H34 * H34') * pO( :,iter+1 ) + sigma_n ) / ( beta2 * trace( H44 * H44') );
    if A4 <= 0
       pQ( :,iter + 1 ) = 0;
    else
       if A4 >= P4_max
   ...
   g2( :,iter ) = ( 1 - beta2 ) * (trace( H22 * H22') * pR( :,iter)  + trace( H12 * H12') * pS( :,iter ) + trace( H31 * H31') * pO( :,iter )+ trace( H42 * H42') * pQ( :,iter ) ) - eL;
   g3( :,iter ) = ( 1 - beta2 ) * (trace( H33 * H33') * pR( :,iter)  + trace( H13 * H13') * pS( :,iter ) + trace( H23 * H23') * pR( :,iter )+ trace( H43 * H44') * pQ( :,iter )) - eL;
   g4( :,iter ) = ( 1 - beta2 ) * (trace( H44 * H44') * pR( :,iter)  + trace( H14 * H14') * pS( :,iter ) + trace( H24 * H24') * pO( :,iter )+ trace( H34 * H34') * pO( :,iter )) - eL;
